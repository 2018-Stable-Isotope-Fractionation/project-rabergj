...
#2D plots of data and model

Since it looks like nearly all the variability is in C:N and d13C, we're just going to focus on that.

```{r}
sim_as_tib <- as_tibble(simulations)

sim_and_data <- 
  ggplot() +
  geom_point(simulations, x = simulations$cn, y = simulations$d13c, aes(color = simulations$d15n)) +
  geom_point(data, x = data$C.N, y = data$d13C, aes(color = data$Age))
data
...
cn_loadings <- loadings[1, 1:3]
d13c_loadings <- loadings[2, 1:3]
d15n_loadings <- loadings[3, 1:3]
```
