...
```{r 3D plot of data alone normalized}
dataplot_norm <- 
  plot_ly() %>% 
  add_markers(qpt_data, x = qpt_data$C.N / mean(qpt_data$C.N), y = qpt_data$d13C / mean(qpt_data$d13C), z = qpt_data$d15N / mean(qpt_data$d15N), color = qpt_data$Age) %>% 
  layout(scene = list(xaxis = list(title = "C:N"),
                      yaxis = list(title = "delta13C"),
                      zaxis = list(title = "delta15N")
                      )) %>% 
...
How do we know how good our model is doing? We'll use the formula for distance on a Cartesian space to see how far the optimal points are from the real data.

$$
distance = \sqrt{(x_{model} - x_{real})^2 + (y_{model} - y_{real})^2 +(z_{model} - z_{real})^2}
$$
But being really far off in $\delta^{13}C$ isn't so bad because there's a lot of variation. Being 2 permille off for carbon is not nearly as bad as being 2 permille off for nitrogen.

To deal with this, we'll also normalize these distances by their errors. It's a little hand-wavy, but should give us a better answer than no error consideration at all.
...
isospace_cn <- 
  ...
  #geom_path(data = qpt_data, aes(x = d13C, y = d15N, color = Age), size = 0.2) +
  scale_color_gradient2(low = "dark blue", mid = "dark green", high = "gold", midpoint = 3500) +
  geom_point(data = source_data_cn, aes(x = Meand13C, y = MeanCN), color = "black") + 
  geom_errorbar(data = source_data_cn, aes(
    x = Meand13C, y = MeanCN, ymin = MeanCN - SDCN, ymax = MeanCN + SDCN)) +
  geom_errorbarh(data = source_data_cn, aes(
    x = Meand13C, y = MeanCN, xmin = Meand13C - SDd13C, xmax = Meand13C + SDd13C)) +
  geom_text(data = source_data_cn, aes(x = Meand13C, y = MeanCN, label = X, hjust = -0.5, vjust = 2)) +
  labs(
    title = "Lake QPT Isospace", 
    color = "Age (yrs)", 
    x = latex2exp::TeX("$\\delta^{13}C$"), 
...
#grid.arrange(isospace, isospace_cn, nrow = 1)
