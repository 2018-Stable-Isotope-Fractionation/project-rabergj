...
qpt_data_plot
#qpt_data_plot <- ggplotly(qpt_data_plot)
#bsiplot
```
...
best_fit <- optim(c(0.3, 0.3, 0.4), i = 3, optim_function, method = "L-BFGS-B", lower = 0, upper = 1)
best_fit
...
optim_results <- 
  ...
  do({
    i <- .$i[1]
    a_optim <- optim(c(0.33, 0.33, 0.34), i = i, optim_function, method = "L-BFGS-B", lower = 0, upper = 1)$par[1]
    t_optim <- optim(c(0.33, 0.33, 0.34), i = i, optim_function, method = "L-BFGS-B", lower = 0, upper = 1)$par[2]
    m_optim <- 1 - a_optim - t_optim
    distance_optim<- optim(c(0.33, 0.33, 0.34), i = i, optim_function, method = "L-BFGS-B", lower = 0, upper = 1)$value
    data_frame(a_optim = a_optim, t_optim = t_optim, m_optim = m_optim, distance_optim = distance_optim)
  })

qpt_data_optim <- 
  bind_cols(qpt_data, optim_results) %>% 
